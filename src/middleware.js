...
export async function middleware(req) {
  ...
  if (
    ...
    console.log("Bypassing auth middleware for:", path);
    return addCORSHeaders(NextResponse.next()); // Add CORS headers to public routes
  } else {
    console.log("Not bypassing auth middleware for:", path);
    if (path.startsWith("/api/transcriptToArticle")) {
      console.log("Receiving request from transcriptToArticle");
    }
  }
  ...
  if (!authToken) {
    console.log("No authToken found, redirecting to /unauthorized");
    return addCORSHeaders(NextResponse.redirect(new URL("/unauthorized", req.url)));
  }
